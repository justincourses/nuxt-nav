<template>
  <TransitionGroup enter-active-class="transition duration-300 ease-out"
    ...
    leave-to-class="transform translate-y-2 opacity-0" class="flex flex-col-reverse gap-4 pointer-events-none">
    <div v-for="toast in toasts" :key="toast.id"
      class="fixed bottom-4 right-4 z-[9999] bg-white shadow-[0_3px_10px_-3px_rgba(6,81,237,0.3)] text-gray-800 flex items-center w-max max-w-sm p-4 rounded-md pointer-events-auto"
      role="alert">
      <svg v-if="toast.type === 'success'" xmlns="http://www.w3.org/2000/svg"
        class="w-[18px] shrink-0 fill-green-500 inline mr-3" viewBox="0 0 512 512">
      ...
      <svg v-else-if="toast.type === 'warning'" xmlns="http://www.w3.org/2000/svg"
        class="w-[18px] shrink-0 fill-yellow-500 inline mr-3" viewBox="0 0 128 128">
        <path
      ...
      <svg v-else-if="toast.type === 'error'" xmlns="http://www.w3.org/2000/svg"
        class="w-[18px] shrink-0 fill-red-500 inline mr-3" viewBox="0 0 32 32">
        <path
          d="M16 1a15 15 0 1 0 15 15A15 15 0 0 0 16 1zm6.36 20L21 22.36l-5-4.95-4.95 4.95L9.64 21l4.95-5-4.95-4.95 1.41-1.41L16 14.59l5-4.95 1.41 1.41-5 4.95z"
          data-original="#ea2d3f" />
      </svg>
      <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-[18px] shrink-0 fill-blue-500 inline mr-3"
