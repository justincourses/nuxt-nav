<script setup>
const isVisible = ref(true);

const handleClose = () => {
  isVisible.value = false;
}

const handleGetOffer = () => {
  window.open('https://buy.stripe.com/fZe9AW506dnr3IY6oq?prefilled_promo_code=XMAS2024', '_blank');
}
</script>

<template>
  <div v-if="isVisible" class="flex items-center max-md:flex-col gap-6 bg-gradient-to-tr from-blue-700 to-purple-400 text-white px-6 py-3.5 mt-4 rounded font-[sans-serif]">
    <p class="text-base flex-1 max-md:text-center">Celebrate the holiday season with our special Christmas offer! Get 50% off on all courses - the perfect gift for learning and growth. Don't miss this festive discount!</p>

    <div>
      <button
        type="button"
        class="bg-white text-blue-500 py-2.5 px-5 rounded text-sm hover:bg-blue-50 transition-colors duration-300"
        @click="handleGetOffer"
        tabindex="0"
        aria-label="Get Offer"
      >
        Get Now
      </button>
      <button
        type="button"
        class="inline-block ml-4"
        @click="handleClose"
        tabindex="0"
        aria-label="Close banner"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-3.5 cursor-pointer fill-white hover:fill-blue-100 transition-colors duration-300"
          viewBox="0 0 320.591 320.591"
        >
          <path
            d="M30.391 318.583a30.37 30.37 0 0 1-21.56-7.288c-11.774-11.844-11.774-30.973 0-42.817L266.643 10.665c12.246-11.459 31.462-10.822 42.921 1.424 10.362 11.074 10.966 28.095 1.414 39.875L51.647 311.295a30.366 30.366 0 0 1-21.256 7.288z"
          />
          <path
            d="M287.9 318.583a30.37 30.37 0 0 1-21.257-8.806L8.83 51.963C-2.078 39.225-.595 20.055 12.143 9.146c11.369-9.736 28.136-9.736 39.504 0l259.331 257.813c12.243 11.462 12.876 30.679 1.414 42.922-.456.487-.927.958-1.414 1.414a30.368 30.368 0 0 1-23.078 7.288z"
          />
        </svg>
      </button>
    </div>
  </div>
</template>
